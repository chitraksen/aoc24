    ...
    | a :: b :: rest -> (
        ...
        | None ->
            ...
            if is_valid_diff a b then
              let new_trend = if a < b then Some true else Some false in
              check_monotonic new_trend (b :: rest)
            else false
        | Some is_increasing ->
            if not (is_valid_diff a b) then false
            else
              (* check trend is maintained *)
              let current_comparison = if is_increasing then a < b else a > b in
...
let rec number_of_safe_reports data dampening =
  ...
  | [] -> 0
  | report :: rest ->
      (if check_report_safety report dampening then 1 else 0)
      + number_of_safe_reports rest dampening
